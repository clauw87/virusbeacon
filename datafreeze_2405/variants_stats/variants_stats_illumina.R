...
# all_pos <- 1:29906
# 
# invar_pos <- all_pos[!(all_pos %in% unique(var$position))]   # 3540
...
vtp_pie 
pdf("/Users/claudiavasallovega/repolab/work/virusbeacon/datafreeze_2405/variants_stats/variant_type_illumina.pdf",  
    height =5.83,  width =8.27)
vtp_pie
dev.off()
...
rcp_pie <- rcp_bp + 
        ...
        theme(axis.text.x = element_blank()) 
rcp_pie 
pdf("/Users/claudiavasallovega/repolab/work/virusbeacon/datafreeze_2405/variants_stats/region_class_illumina.pdf",  
    height =5.83,  width =8.27)
rcp_pie 
...
vep_pie <- vep_bp  + 
        ...
        theme(axis.text.x = element_blank()) 
vep_pie 
pdf("/Users/claudiavasallovega/repolab/work/virusbeacon/datafreeze_2405/variants_stats/var_effect_illumina.pdf",  
    height =5.83,  width =8.27)
vep_pie 
...
pdf("/Users/claudiavasallovega/repolab/work/virusbeacon/datafreeze_2405/variants_stats/fun_class_illumina.pdf",  
    height =5.83,  width =8.27)
fcp_pie 
...
illum_ppm_sites <- unique(uvar$position)[sapply(illum_alt_per_pos, function(a){
        length(a)>1| sum(iupac %in% a)>0
})]
length(illum_ppm_sites) # 15231
head(illum_ppm_sites) # 1 2 3..
...
length(unique(intersect(illum_ppm_sites, consensus)))
...
#         labs(y="Freq")
af
pdf("/Users/claudiavasallovega/repolab/work/virusbeacon/datafreeze_2405/variants_stats/illumina_af.pdf",
    height =5.83,  width =8.27)
#af
...
title <- "RUN COUNTS VS POSITION COUNT IN DATASET"
...
mm3 <- ggplot(dat,
              aes(x=run_var/1497, y=af, color=class)) +
        #ggplot(data=dat, aes(x=pos, y=meanfreq, ymin=minfreq, ymax=maxfreq)) +
        geom_point(size=1) +  #, color="dodgerblue
        scale_color_manual(values = c("green", "orangered2", "blue")) +
        theme_minimal() +
...
# dat <- data.frame(pos=uvar$position, freq=uvar$MyFrequency)
pvar <- filter(var, position %in% illum_pm_sites)
dat <- data.frame(pos=pvar$position, freq=pvar$PosCount/1497, class=pvar$Functional_Class, stringsAsFactors = F)
#dat <- data.frame(pos=var_data$position, freq=var_data$dbfreq, class=var_data$Functional_Class, stringsAsFactors = F)
dat$class[which(dat$class=="MISSENSE"|dat$class=="NONSENSE")] <- "NS"
dat$class[which(dat$class=="SILENT")] <- "S"
dat$class[which(dat$class=="")] <- "NC"
